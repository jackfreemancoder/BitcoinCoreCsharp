...
namespace BitcoinCore.JsonConverters
{
	public class HDFingerprintJsonConverter : JsonConverter
	{
		...
		public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
		{
			try
			{
				...
				if (reader.TokenType == JsonToken.String)
					return new HDFingerprint(BitcoinCore.DataEncoders.Encoders.Hex.DecodeData((string)reader.Value));
		...
		public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
		{
			if (value != null)
			{
				writer.WriteValue(BitcoinCore.DataEncoders.Encoders.Hex.EncodeData(((HDFingerprint)value).ToBytes()));
			}
		}
	}
}
