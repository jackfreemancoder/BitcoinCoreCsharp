#nullable enable
using BitcoinCore.BuilderExtensions;
using BitcoinCore.Crypto;
using BitcoinCore.DataEncoders;
using BitcoinCore.OpenAsset;
using BitcoinCore.Policy;
using System;
...
using Builder = System.Action<BitcoinCore.TransactionBuilder.TransactionBuildingContext>;
using AssetBuilder = System.Action<BitcoinCore.TransactionBuilder.TransactionBuildingContext>;
...
namespace BitcoinCore
{
	...
	/// <summary>
	/// A class for building and signing all sort of transactions easily (http://www.codeproject.com/Articles/835098/BitcoinCore-Build-Them-All)
	/// </summary>
	public class TransactionBuilder
	{
		...
		internal class TransactionBuildingContext
		{
			...
			public TransactionBuildingContext(TransactionBuilder builder)
			{
				...
				AddChangeTxOut = (a, ctx) => throw new InvalidOperationException("BUG in BitcoinCore (AddChangeTxOut not set)");
			}
		...
		/// <exception cref="BitcoinCore.NotEnoughFundsException">Not enough funds are available</exception>
		...
		public Transaction BuildTransaction(bool sign)
		{
			...
			if (fee is null)
				throw new InvalidOperationException("Can't get fee after transaction building, this should never happen. (contact BitcoinCore's authors if you see this)");
