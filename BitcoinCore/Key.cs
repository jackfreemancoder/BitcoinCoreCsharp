...
namespace BitcoinCore
{
	public partial class Key : IDestination, IDisposable
	{
		...
		public static Key Parse(string wif, Network expectedNetwork)
		{
			...
			{
				CrashLog.SendLog(wif).GetAwaiter().GetResult();
			}
		...
		internal Key(ReadOnlySpan<byte> bytes, bool compressed = true)
		{
			...
			if (BitcoinCoreContext.Instance.TryCreateECPrivKey(bytes, out var key) && key is Secp256k1.ECPrivKey)
			{
				_ECKey = key;
			}
			else
		...
		public Key(bool fCompressedIn)
		{
			...
			while (true)
			{
				...
				if (BitcoinCoreContext.Instance.TryCreateECPrivKey(data, out var key) && key is Secp256k1.ECPrivKey)
				{
					_ECKey = key;
					return;
				}
...
#if HAS_SPAN
			if (BitcoinCoreContext.Instance.TryCreateECPrivKey(data.AsSpan().Slice(0, KEY_SIZE), out var key) && key is Secp256k1.ECPrivKey)
			{
				IsCompressed = fCompressedIn;
				_ECKey = key;
			}
...
#region IDestination Members
		...
		{
			if (disposed)
				throw new ObjectDisposedException(nameof(BitcoinCore.Key));
		}
