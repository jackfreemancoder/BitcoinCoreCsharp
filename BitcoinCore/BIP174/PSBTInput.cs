...
using PartialSigKVMap = System.Collections.Generic.SortedDictionary<BitcoinCore.PubKey, BitcoinCore.TransactionSignature>;
using System.Diagnostics.CodeAnalysis;
using BitcoinCore.BIP370;
...
namespace BitcoinCore
{
	...
	public abstract class PSBTInput : PSBTCoin
	{
		...
		internal bool TryFinalizeInput(SigningOptions? signingOptions, [MaybeNullWhen(true)] out IList<PSBTError> errors)
		{
			...
			if (coin is null)
				throw new InvalidOperationException("Bug in BitcoinCore during TryFinalizeInput: Please report it");
		...
		public bool VerifyScript(ScriptVerify scriptVerify, PrecomputedTransactionData? precomputedTransactionData, out ScriptError err)
		{
			...
			if (GetTransaction() is IHasForkId)
				eval.ScriptVerify |= BitcoinCore.ScriptVerify.ForkId;
			var txout = GetTxOut();
			if (txout is null)
			{
